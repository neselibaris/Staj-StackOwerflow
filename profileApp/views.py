...
def userActivity(request ,user_uuid):
    ...
    return render(request, 'profile-user-activity.html', context)
...
def award_badge(customuser, badge_id):
    badge = Badge.objects.get(pk=badge_id)
    customuser.badge_count += 1
    customuser.save()
    customuser.badge.add(badge)
